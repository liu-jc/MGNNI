...
def get_dp_solution(nums):
    # print(nums)
    l = len(nums)
    solutions = [[] for i in range(l)] # the solution contains the index at each step.
    ...
    for i in range(2,l):
        if dp[i-1] > dp[i-2] + nums[i]:
            ...
            dp[i] = dp[i-2] + nums[i]
            solutions[i] = solutions[i-2] + [i]
    # print(f'Whole dp list: {dp}')
    # print(f'The final solution: {solutions[-1]}')
    return dp[-1], solutions[-1]
...
def preprocess_adj(adj, normalization="FirstOrderGCN"):
...
def load_citation_IDM(dataset_str="cora", normalization="AugNormAdj", cuda=True, need_orig=False):
    """
    Load Citation Networks Datasets.
    """
    if 'chains' in dataset_str:
        return load_citation_chain_IDM(normalization, cuda, need_orig=need_orig)
...
def load_citation_chain_IDM(normalization, cuda, need_orig=False):
...
def load_raw_graph_IDM(dataset_str = "amazon-all"):
    txt_file = 'data/' + dataset_str + '/adj_list.txt'
    graph = {}
    with open(txt_file, 'r') as f:
        cur_idx = 0
...
def load_txt_data_IDM(dataset_str = "amazon-all", portion = '0.06'):
    sp_adj, adj = load_raw_graph_IDM(dataset_str)
    idx_train = list(np.loadtxt('data/' + dataset_str + '/train_idx-' + str(portion) + '.txt', dtype=int))
    idx_val = list(np.loadtxt('data/' + dataset_str + '/test_idx.txt', dtype=int))
    idx_test = list(np.loadtxt('data/' + dataset_str + '/test_idx.txt', dtype=int))
    labels = np.loadtxt('data/' + dataset_str + '/label.txt')
    ...
    return sp_adj, adj, features, labels, idx_train, idx_val, idx_test, num_nodes, num_class
...
def indicator_adj(adj):
    # out = adj.copy().tocsr()
    rows, cols, _ = sp.find(adj >= 1.0)
    vals = np.ones(rows.shape[0], dtype=np.float)
    out = sp.csr_matrix((vals, (rows,cols)), shape=adj.shape)
    # ipdb.set_trace()
    # for r, c in zip(rows, cols):
    #     out[r, c] = 1.0
    # vals = np.ones(rows.shape[0], dtype=np.float)
    # out = sp.csr_matrix((vals, (rows,cols)), shape=adj.shape)
    # rows, cols, _ = sp.find(adj < 0.0)
    # for r, c in zip(rows, cols):
    #     out[r, c] = 0.0
    # ipdb.set_trace()
    return out.tocoo()

def pair_wise_cosine_sim(h1, h2):
    # h1 and h2 are torch tensors. (n, d)
    # return a matrix (n, n)
    h1_norm = h1 / torch.norm(h1, dim=1, keepdim=True)
    h2_norm = h2 / torch.norm(h2, dim=1, keepdim=True)
    output = h1_norm @ h2_norm.T
    return output

def get_avg_cosine_sim(cosine_matrix):
    cosine_matrix = cosine_matrix.fill_diagonal_(0.0)
    return cosine_matrix.mean().item()