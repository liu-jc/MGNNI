...
from utils import load_citation, accuracy, clip_gradient, l_1_penalty, get_spectral_rad
from models import *
...
parser.add_argument('--num_layers', type=int, default=2, help='for encoder after fix_point layer and also the decoder for graph representations')

parser.add_argument('--gamma', type=float, default=0.8)
parser.add_argument('--path', type=str, default='./results_graph_cls/')
...
result_name = f'EIGNN_m_att_{args.dataset}_{args.epochs}_{args.hidden}_{ks_str}_l{args.num_layers}_lr{args.lr}_dp{args.dropout}_{args.gamma}_seed{args.seed}.txt'
...
for fold_idx in range(10):
    ...
    model = MGNNI_m_att_stack(m=dataset.num_features,
                              nhid=args.hidden,
                              m_y=dataset.num_classes,
...
filep.write('Graph classification mean accuracy and std are {}\n'.format(re_all))